...
public class MessageRepository(DataContext context, IMapper mapper) : IMessageRepository
{
    ...
    public async Task<PagedList<MessageDto>> GetMessagesForUser(MessageParams messageParams)
    {
        ...
        {
            "Inbox" => query.Where(x => x.Recipient.UserName == messageParams.Username && x.RecientDeleted==false),
            "Outbox" => query.Where(x => x.Sender.UserName == messageParams.Username && x.SenderDeleted==false),
            _ => query.Where(x => x.Recipient.UserName == messageParams.Username && x.DateRead == null && x.RecientDeleted==false)
        };
    ...
    public async Task<IEnumerable<MessageDto>> GetMessageThread(string currentUsername, string recipientUsername)
    {
        ...
        .Where(x =>
                x.RecipientUsername == currentUsername && x.RecientDeleted == false && x.SenderUsername == recipientUsername ||
